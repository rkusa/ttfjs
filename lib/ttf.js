...
var TTFFont = module.exports = function(path) {
  ...
  function unpackTable(table, args) {
    ...
    if (Table.length > 1) Table = Table.apply(undefined, args)
    self.tables[name] = self.tables[table] = (new Table).unpack(view)
  }
  
  unpackTable('cmap')
  console.log(this.tables.cmap.subtables[0])
  // unpackTable('glyf')
  unpackTable('head')
  unpackTable('hhea')
  unpackTable('maxp')
...
TTFFont.prototype.save = function() {
  ...
  // prepare cmap
  for (var i = this.tables.cmap.subtables.length - 1; i >= 0; --i) {
    if (!('codeMap' in this.tables.cmap.subtables[i].table)) {
      this.tables.cmap.subtables.splice(i, 1)
    }
  }
