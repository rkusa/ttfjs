...
Cmap.prototype.embed = function(cmap) {
  ...
  var nextId = 0, map = {}, charMap = {}, last = diff = null, segments = []
  codes.forEach(function(code) {
    ...
    if (last === null || delta !== diff) {
      if (last) {
        segments[segments.length - 1].end = last
      }
      segments.push({ start: +code, end: 0xFFFF })
      diff = delta
    }
    last = +code
  })
  ...
  segments = segments.sort(function(lhs, rhs) {
  ...
  segments.forEach(function(segment) {
    var startCode = segment.start
    var endCode = segment.end
  ...
  var subtable = new SubTable({
    ...
    table: new Table({
      ...
      endCode: segments.map(function(seg) { return seg.end }),
      reservedPad: 0,
      startCode: segments.map(function(seg) { return seg.start }),
      idDelta: deltas,
